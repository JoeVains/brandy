...
function ToolbarButton({ active, onClick, title, children }: { active?: boolean; onClick: () => void; title: string; children: React.ReactNode }) {
  return (
    <button
      ...
      style={active ? { background: 'var(--accent)', color: 'white' } : { color: '#6b7280' }}
    >
      {children}
    </button>
  );
...
export default function TextModule({ module, brandColor, onUpdate, isEditing: isEditMode }: Props) {
      ...
      <div className="space-y-2">
        <div className="border rounded-xl overflow-hidden" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-0.5 px-2 py-1.5 border-b flex-wrap" style={{ borderColor: 'var(--border)' }}>
            <ToolbarButton title="Gras" active={activeFormats.bold} onClick={() => exec('bold')}><Bold size={14} /></ToolbarButton>
            <ToolbarButton title="Italique" active={activeFormats.italic} onClick={() => exec('italic')}><Italic size={14} /></ToolbarButton>
            <ToolbarButton title="Souligné" active={activeFormats.underline} onClick={() => exec('underline')}><Underline size={14} /></ToolbarButton>
            <ToolbarButton title="Barré" active={activeFormats.strikeThrough} onClick={() => exec('strikeThrough')}><Strikethrough size={14} /></ToolbarButton>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--border)' }} />
            <ToolbarButton title="Exposant" active={activeFormats.superscript} onClick={() => exec('superscript')}><Superscript size={14} /></ToolbarButton>
            <ToolbarButton title="Indice" active={activeFormats.subscript} onClick={() => exec('subscript')}><Subscript size={14} /></ToolbarButton>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--border)' }} />
            <ToolbarButton title="Majuscules" active={activeFormats.uppercase} onClick={toggleUppercase}><span className="text-xs font-bold leading-none px-0.5">AA</span></ToolbarButton>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--border)' }} />
              ...
              {linkMenuOpen && (
                <div className="absolute left-0 top-full mt-1.5 rounded-xl border shadow-lg p-2.5 z-50 w-56" style={{ borderColor: 'var(--border)', background: 'var(--card-bg)' }}>
                  ...
                  <div className="flex gap-2">
                    <button onMouseDown={e => e.preventDefault()} onClick={applyLink} className="flex-1 px-2 py-1 rounded-lg text-xs text-white" style={{ background: brandColor }}>
                      Appliquer
                    </button>
            ...
            <div className="w-px h-5 mx-1" style={{ background: 'var(--border)' }} />
            <ToolbarButton title="Effacer la mise en forme" onClick={() => exec('removeFormat')}><Eraser size={14} /></ToolbarButton>
          </div>
          <div
            ref={editorRef}
            contentEditable
  ...
  return (
    <div className="relative">
      {module.content ? (
        <div
          className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed [&_a]:underline [&_a]:text-blue-600"
