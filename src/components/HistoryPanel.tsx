...
export default function HistoryPanel({ onClose }: Props) {
  ...
  return createPortal(
    <>
      ...
      {/* Panel */}
      <div className="fixed top-0 right-0 bottom-0 z-50 w-80 bg-white shadow-2xl flex flex-col border-l"
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="flex items-center justify-center h-32 text-xs text-gray-400">Chargement…</div>
          ) : entries.length === 0 ? (
            <div className="flex items-center justify-center h-32 text-xs text-gray-400">Aucun historique</div>
          ) : (
            <div className="py-2">
              {entries.map((entry, i) => {
                const meta = ACTION_META[entry.action];
                return (
                  <div key={entry.id} className="flex gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors">
                    {/* Timeline dot */}
                    ...
                    <div className="flex-1 min-w-0 pb-1">
                      ...
                      {entry.entityName && (
                        <p className="text-xs text-gray-500 truncate">
                          {entry.entityName}
                          {entry.entityType && <span className="text-gray-300 ml-1">· {entry.entityType}</span>}
                        </p>
                      )}
                      <div className="flex items-center gap-1.5 mt-0.5">
                        {entry.brandName && (
                          <span className="text-[10px] text-gray-300 truncate">{entry.brandName}</span>
                        )}
                        {entry.brandName && entry.sectionName && (
                          <span className="text-[10px] text-gray-200">·</span>
                        )}
                        {entry.sectionName && (
                          <span className="text-[10px] text-gray-300 truncate">{entry.sectionName}</span>
                        )}
                      </div>
                      <p className="text-[10px] text-gray-300 mt-0.5">{relativeTime(entry.timestamp)}</p>
                    </div>
                  </div>
                );
              })}
